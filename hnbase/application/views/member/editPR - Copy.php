...
<?php    echo   form_open_multipart('backoffice/all_update_form');	  ?>
...
//======================  เปลี่ยนหน้าตาม page ของจำนวนfiled
			   //##----- นิยามตัวแปร
					...
					//  $va_enable=1;
					$tb="tb_new";
				//	 $list_limit=3;  //ให้ดึงเป็นค่าเดียวกับ  	$data['limit']=5;    
					//$link_control= base_url()."index.php/home/change_page"; 
					//change_page_admin
					$link_control= base_url()."index.php/backoffice/change_page_admin"; 
					$id_enable='new_enable';
				    $va_enable=1;
...
if( $sr == '' )
{				
      $this->querymodels->query_new(1,$limit,$id,$tb,$ofset,$link_anchor_popup); //1=ปักหมุด,ตัวต่อมาคือ limit ในการแสดงผล  
      $this->querymodels->query_new(0,$limit,$id,$tb,$ofset,$link_anchor_popup); //0=ไม่ปักหมุด,ตัวต่อมาคือ limit ในการแสดงผล 
}
elseif ( strlen($sr) > 0 )
{
     $link_anchor_popup="backoffice/show_popup_picture/";
	 $this->querymodels->news($query_search,$link_anchor_popup); 
}	  
