        ...
        {//begin
          if(  $this->ck_login()  ==   1   &&  $this->ck_permission() == 1  )
            {//if  
               ...
               // echo br();                                                                   
                $count_experience=trim($this->input->get_post('count_experience'));
                //echo br();             
                 $before_workplace=trim($this->input->get_post('before_workplace'));
                //echo br(); 
                $workplace=trim($this->input->get_post('workplace'));
               // echo br(); 
                 $work_type=trim($this->input->get_post('work_type'));
                //echo br();
                 $count_year=trim($this->input->get_post('count_year'));
                //echo br();                               
                $tb="tb_experience";              
               
                $data_doc=array(
                    'id_tb_experience'=>NULL,
                    'id_tb_person'=>$id_tb_person,
                    'count_experience'=>$count_experience,
                    'before_workplace'=>$before_workplace,
                    'workplace'=>$workplace,
                    'work_type'=>$work_type,
 ...
 function   detail()//เรียกดูข้อมูลของพนักงานทั้งหมดจาก popup หน้ารวม
        {
          ...
          on  `tb_person`.id_tb_person=`tb_relation_person`.id_tb_person where tb_relation_person.id_tb_person=".$id_tb_person."";
                       ...
                       //$data['path']="upload/";
                       // $fieldset="";
                       $this->db->order_by("id_tb_experience", "desc");
                       ...
                       $link_delete=base_url()."index.php/experience/delete_tb_experience/";
                       //$this->tableQuerymodel->tb_experience_models_popup($query,$send_page,$link_delete);
                       $this->tbload->tb_experience_models_popup($query,$send_page,$link_delete);
                   } //end if                
              }
              else
               {
            ...
            if(  $this->ck_login()  ==   1  && $this->ck_permission() == 1   )
             {
                   ...
                   if (is_numeric($id_tb_experience)) 
                   {
                     ...
                     //  $this->db->update($tb,$data_update);
                       $this->db->where('id_tb_experience', $id_tb_experience);
