...
class Article extends CI_Controller {
   ...
   function  crt_id_user()  //ใช้ check การ login ของระบบ
	...
	function  article_menu()  //main menu สำหรับ  ประชาสัมพันธ์
	{
...
//									   $this->load->view('admin/home_admin2',$data);  //10
									 ...
									 $data['right_content']='admin/article/insert_article'; 
                             ...
                             case  4:  //โหลดเพื่อทำการ updte หน้า pr
                                {
															 ...
															 $data['right_content']='admin/article/update_article'; 
																   ...
																   //##------------- query  tb_new ------------------------ 
																    ...
																    $data['id_content_article']=$id;					
																									     $tb="tb_content_article";
																										 $id_f="id_content_article";
																									     ...
																									     $num_check = $query->num_rows();
																												  if( $num_check == 1 )
																												  {
																														...
																														$data['id_article']=$row->id_article;
																														
																														$data['data_title']=$row->title;  //หัวข้อข่าวสาร
																														
																														$data['data_pin_content']=$row->pin_content;  //0=ไม่ปัก  1=ปัก
																														$data['data_picture_content_article']=$row->picture_content_article;  //รูปภาพ
																														$data['alius_pic']=   $link_anchor_popup.''.$data['data_picture_content_article'];  
																														 $data['content']=$row->content;  //ข่าวสาร
																														 $data['data_content_enable']=$row->content_enable;   //1=show,0=hidden 
...
##============== insert========================
function   insert_article()  //เพิ่มข่าวสาร
{
	if( 	 $this->ck_login()  ==   1    )
	{  //if
		   $id_article=$this->input->get_post('id_article');
		 // echo br();
		  $title=trim($this->input->get_post('title'));
		  $pin_pr=$this->input->get_post('pin_content');
		  if( $pin_pr == '' ) {  $pin_pr=0;  }   //ปักหมุด
		  
//-----------------------upload  picture function------------------------
  $tumbnail=$_FILES['picture_content_article']['tmp_name'];
  $tumbnail_name=$_FILES['picture_content_article']['name'];
  $tumbnail_size=$_FILES['picture_content_article']['size'];
  $tumbnail_type=$_FILES['picture_content_article']['type'];
						if(  $tumbnail_name != '' )
						{
...
//----------------------------------------------------------------------------------------------------	
 	  ...
 	  // echo   $DMY=mdate($datestring,$time);
	     ...
	     $this->crt_id_user();
$data = array(
   'id_content_article' => NULL,
   'id_article'=>$id_article,
   'title' =>$title ,
   'picture_content_article' => $tumbnail_name,
   'content'=>$content,
   'DMY'=>$DMY,
   'content_enable'=>1,  //1=show,0=hidden
   'id_user'=>$this->crt_id_user(),
    'pin_content'=>$pin_pr
);
					
						$record_tb_new=$this->db->insert('tb_content_article', $data);  // insert   tb_new
							// $record_tb_new=true;
							if(   $record_tb_new   )
							{
							       //echo  "T";
								     redirect('article/article_menu/3');
							}
	   ...
	   {  //if
                  ...
                  //$this->querymodels->search_keyword($sr);  
					...
					// $field="content";
					 $field="title";
					 $limit=5;
					 $id="id_content_article";  //ใช้สำหรั้บ order_by
					//$ofset=0;
					
				//	$url_redirect="backoffice/admin_leftmenu/2";
					$url_redirect="article/article_menu/3";
					...
					if( $ck_num > 0  ||   strlen($sr) > 0  )
					{
									...
									//  $this->querymodels->search_pr_show($sr,$query_search);
									    $this->queryarticle->search_article_show($sr,$query_search);
...
function  query_article_update()//--- update   tb_new
{
       				if( 	 $this->ck_login()  ==   1    )
					{  //if
						...
						{  //if
					                  $id= $this->uri->segment(3);
									   $tb=  $this->uri->segment(4);
									     if( $id>0   )
										 {
														$this->db->where('id_content_article',$id);
												    	$this->db->delete($tb);
														 redirect('/article/article_menu/3','refresh');
...
function  update_article()//--- update   tb_new
{
       				if( 	 $this->ck_login()  ==   1    )
					{  //if
                   		          $id_content_article=$this->input->get_post('id_content_article');
								echo   $id_article=$this->input->get_post('id_article');
								echo  br();
								   $pin_content=$this->input->get_post('pin_content');
							   //   $id_tb_pr=trim($this->input->get_post('id_tb_pr'));
								//echo br();
								    $title=trim($this->input->get_post('title'));
								//echo br();
			...
			//echo br();
				...
				/*######### -------------- ระบบรูปภาพ และการ upload ---------------------------- */
		 						//  $pin_pr=$this->input->get_post('update_pin_pr');
								   if(  $pin_content == '' ) {  $pin_pr=0;  }   //ปักหมุด
								 //echo  $pin_new;
								 //echo  br();
		 	
        $content=$this->input->get_post('content');
        $content_enable=$this->input->get_post('content_enable');
	              if(  $content_enable == '' ) {  $content_enable =0;  }   //ปักหมุด  //1=show,0=hidden
				   //echo   $new_enable;
         //$datestring = "%Y-%m-%d %d:%h:%i";
		 //$time = time();
	     //$DMY=mdate($datestring,$time);
				...
				$data_update = array(
				  ...
				  // 'new_picture' => $tumbnail_name,  //ok
				   'content'=>$content, //ok
				    'id_article'=>$id_article,
				  // 'DMY'=>$DMY,
				   'content_enable'=>$content_enable,  //1=show,0=hidden
				   'id_user'=>$this->crt_id_user(),
...
//---------------- upload function ----------------------------------------------------				
				$data_update = array(
				   'title' =>$title ,   //ok
				   'id_article'=>$id_article,
				   'picture_content_article' => $tumbnail_name,  //ok
				   'content'=>$content, //ok
				  // 'DMY'=>$DMY,
				   'content_enable'=>$content_enable,  //1=show,0=hidden
				   'id_user'=>$this->crt_id_user(),
					'pin_content'=>$pin_content
							      ...
							      //$record_tb_new=$this->db->insert('tb_new', $data);  // insert   tb_new
                                  //http://localhost/hostKKHP/index.php/http://localhost/hostKKHP/index.php/backoffice/admin_leftmenu/2
								 $this->db->where('id_content_article',  $id_content_article);
								  $check_update=$this->db->update('tb_content_article', $data_update); 
								 // $check_update=TRUE;
								  if(  $check_update == TRUE )
								  {
										//echo "T";
