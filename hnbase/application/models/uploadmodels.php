...
class Uploadmodels extends  CI_Model {
    function Uploadmodels()  //model  การ query ทั้งหมด
    {
        //parent::CI_Model();
		   parent::__construct();
    }
...
//----------------------------------------------------------------------------------------------------	
   ...
   function  upload_file_hn($file_wd,$path)
   {//begin
       if( !empty($file_wd) )
       {//if
             $source = $_FILES[$file_wd]['tmp_name'];
            //$source = $file_wd.['tmp_name'];
            //$target = "upload/".$_FILES['doc_38_1']['name'];
              $target=$path.$_FILES[$file_wd]['name'];
              return $upload_1=move_uploaded_file( $source, $target );// or die ("Couldn't copy");          
       }//end if
   }//end
