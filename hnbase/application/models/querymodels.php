...
//##--- ทดสอบ query autocomplete  ---------------	
    ...
    {
        $name = '%' . $post["term"] . '%';
        $sql = "SELECT * FROM countries WHERE country LIKE :name";
...
//###==============================PR===========================================================	
		function     query_pr_all_content($tb,$id,$limit,$ofset)//หน้า    ข่าวรวม   อ่านข่าวทั้งหมดจากหน้าหลัก    ไม่มีการเลือกว่าปักหมุดหรือไม่ปัก
	{
	       			//##----------  ตัวแปร ------------
					//  $tb="tb_new";  //ชื่อ table
					 //  $id="id_tb_new";  //ชื่อ  id  ของ table
					 ...
					 //  $ofset=1;
					  //##----------  ตัวแปร ------------ 
						...
						//echo  $ofset;
						if( $ofset  < 0 ) {  $ofset = 0;  }
						 $query=$this->db->get_where($tb,array('pr_enable'=>1), $limit,$ofset);
						...
						if( $row_check  > 0 )
						{
													   foreach($query->result() as $row)
													   {
															   ...
															   $content=$row->content;
																...
																if( strlen($pr_picture) > 0 )  //###############------------------- รูปภาพ
																{
																				echo "<p>";
																				echo"<img width=\"100\" height=\"100\" alt=\"\" src=\"".base_url()."picture/".$pr_picture."\" style=\"float:left;margin-right:10px;\" />";
																				echo  "</p>";
																}
																else  //#####------------ กรณีไม่ได้ up รูปภาพตัวอย่าง
																{
															 ...
															 echo "<p style=\"font-weight: bold;\">".DMY_thai_convert($DMY)."</p>";    //DMY_thai_convert($DMY);  //MY_helpers    วัน เดือน ปี  เวลา
															  ...
															  echo "<img src=\"".base_url()."images/dot1.gif\">";  
															  echo  nbs(1);
															  echo     "<a href=\"".base_url()."index.php/home/link_read_new/".$id_tb_pr."/home/test/\"   target=_blank  >".$title."</a>";
															  //echo  "</p>";
...
*/																							  
  ...
  function    search_pr_show($sr,$query_search)//backoffice/admin_leftmenu/2
  {
                                                       //##----  ตัวแปร
													   //$sr=  $sr=$this->input->get_post('search_');  มาจาก textbox ในการค้นหา
													   // $query_search  มาจาก   	 $this->db->order_by($id, "desc");  $this->db->like($field,$sr);   $query_search=$this->db->get($tb);
...
//																  $data['right_content']="admin/news/editnew";
	...
	function  query_pr($pin_status,$limit,$id,$tb,$ofset,$link_anchor_popup)  //  Admin ข่าวสาร NEWS  query  ข่าว  1=ปักหมุด,0=ไม่ปักหมุด   
	{
           //###------- ตัวแปร------
		        //$pin_status=1;    //1=ปักหมุด,0=ไม่ปักหมุด    
			//	echo  br();
          ...
          'rel' => 'lightbox',
					 ...
					 { //##------------ begin foreach-----------------------------------------------
					        $id_tb_pr=$row->id_tb_pr;
							?>
							<input name="hid_id_tb_pr[]" id="hid_id_tb_pr[]" type="hidden" value="<?php echo $id_tb_pr; ?>" />
                            <?php
							$pr_picture=$row->pr_picture;
							$alius_pic  =   $link_anchor_popup.''.$pr_picture;
							$image_properties = array(
          'src' => 'picture/'.$pr_picture,
          'alt' => '',
          'class' => '',
          'width' => '50',
          'height' => '50',
          ...
          'rel' => 'lightbox',
		     ...
		     $pr_enable	=$row->pr_enable;  //แสดงผล  0=ไม่แสดง,1=แสดง
			 $pin_pr=$row->pin_pr;   //1=ปักหมุด, 0=ไม่ปักหมุด
...
-->
<tr onmouseover="this.style.backgroundColor='#ffff66';" onmouseout="this.style.backgroundColor='#FFFFFF';">
<td><?php echo  $id_tb_pr; ?></td>
<td>
<!--<img src="<?php echo base_url();?>picture/<?php echo  $pr_picture;  ?>" width="50" border="1">-->
<?php   

if( $pr_picture != "" )
{
			?>
                <img src="<?php echo  base_url(); ?>picture/<?php   echo  $pr_picture;  ?>" width="50" height="50"/>
            <?php
}
else
{
...
$image_edit_properties = array(
 ...
 echo  nbs(1);
?>
<!--<a href="<?=base_url()?>index.php/pr/send_editform/1/<?php echo   $id_tb_pr;  ?>">แก้ไข</a>-->
<a href="<?=base_url()?>index.php/pr/pr_menu/4/<?php echo   $id_tb_pr;  ?>">แก้ไข</a>
<?php
$image_edit_properties = array(
          'src' => 'images/de4-re.jpg',
          'alt' => '',
...
<!--<a href="#"  onclick="check_del(<?php echo  $id_tb_pr;  ?>)"  >ลบ</a>-->
<!--<a href="<?=base_url()?>index.php/pr/delete_table/<?php echo  $id_tb_pr;  ?>/tb_pr"  onclick="check_del()"  >ลบ</a>-->
<a href="#"  onclick="check_del(<?php echo  $id_tb_pr;  ?>)"  >ลบ</a>
...
<a href="<?=base_url()?>index.php/home/link_read_pr/<?=$id_tb_pr?>" target="_blank" >แสดงตัวอย่าง</a>
...
<td>
<?php  if( $pr_enable  == 1  )    // 1=แสดงผล,0=ไม่แสดงผล
      {
?>
<input name="check_enable[]" id="check_enable[]" type="checkbox"    value="1" checked="checked" />
<?php   
...
function  query_pr_sr($query_search,$link_anchor_popup)  //  Admin ข่าวสาร NEWS  query  ข่าว  1=ปักหมุด,0=ไม่ปักหมุด   
	{
           //###------- ตัวแปร------
		        //$pin_status=1;    //1=ปักหมุด,0=ไม่ปักหมุด    
			//	echo  br();
...
<?php
        ...
        function check_val_name($btn,$txt,$str)//ใช้สำหรับตรวจสอบชื่อก่อนทำการบันทึกข้อมูล
        {
           ?>                                                                    
            <script>
                $(function()
